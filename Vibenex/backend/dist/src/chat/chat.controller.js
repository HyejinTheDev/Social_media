...
let ChatController = class ChatController {
    ...
    getOrCreateConversation(req, userId) {
        return this.chatService.getOrCreateConversation(req.user.sub, userId);
    }
    getMessages(req, conversationId, page) {
        return this.chatService.getMessages(conversationId, req.user.sub, page ? parseInt(page, 10) : 1);
    }
    sendMessage(req, conversationId, body) {
        return this.chatService.sendMessage(conversationId, req.user.sub, body.content, body.imageUrl);
    }
    markAsRead(req, conversationId) {
        return this.chatService.markAsRead(conversationId, req.user.sub);
    }
    getUnreadCount(req, conversationId) {
...
__decorate([
    (0, common_1.Post)('conversations/:userId'),
    (0, swagger_1.ApiOperation)({ summary: 'Get or create conversation with a user' }),
    __param(0, (0, common_1.Request)()),
...
__decorate([
    (0, common_1.Get)('conversations/:conversationId/unread'),
