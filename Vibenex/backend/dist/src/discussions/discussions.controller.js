"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DiscussionsController = void 0;
const common_1 = require("@nestjs/common");
const discussions_service_1 = require("./discussions.service");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
let DiscussionsController = class DiscussionsController {
    discussionsService;
    constructor(discussionsService) {
        this.discussionsService = discussionsService;
    }
    findByChannel(channelId, page = '1', limit = '20') {
        return this.discussionsService.findByChannel(channelId, parseInt(page), parseInt(limit));
    }
    create(req, channelId, body) {
        return this.discussionsService.create(channelId, req.user.sub, body);
    }
    findOne(id) {
        return this.discussionsService.findById(id);
    }
    getReplies(discussionId) {
        return this.discussionsService.getReplies(discussionId);
    }
    createReply(req, discussionId, body) {
        return this.discussionsService.createReply(discussionId, req.user.sub, body);
    }
};
exports.DiscussionsController = DiscussionsController;
__decorate([
    (0, common_1.Get)('channels/:channelId/discussions'),
    __param(0, (0, common_1.Param)('channelId')),
    __param(1, (0, common_1.Query)('page')),
    __param(2, (0, common_1.Query)('limit')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object, Object]),
    __metadata("design:returntype", void 0)
], DiscussionsController.prototype, "findByChannel", null);
__decorate([
    (0, common_1.Post)('channels/:channelId/discussions'),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Param)('channelId')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, Object]),
    __metadata("design:returntype", void 0)
], DiscussionsController.prototype, "create", null);
__decorate([
    (0, common_1.Get)('discussions/:id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], DiscussionsController.prototype, "findOne", null);
__decorate([
    (0, common_1.Get)('discussions/:discussionId/replies'),
    __param(0, (0, common_1.Param)('discussionId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], DiscussionsController.prototype, "getReplies", null);
__decorate([
    (0, common_1.Post)('discussions/:discussionId/replies'),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Param)('discussionId')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, Object]),
    __metadata("design:returntype", void 0)
], DiscussionsController.prototype, "createReply", null);
exports.DiscussionsController = DiscussionsController = __decorate([
    (0, common_1.Controller)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:paramtypes", [discussions_service_1.DiscussionsService])
], DiscussionsController);
//# sourceMappingURL=discussions.controller.js.map